...
begin
  ...
  frmEfeito.lblMensagem.Caption := 'O Cor Sólida está ativo neste momento.';
...
begin
  ...
  begin
    ShowMessage('Você selecionou a mesma cor que estava!');
    btnSalvar.Enabled := False;
  end;
...
if(tbLuminCor.Position <> dmPrincipal.adCor.FieldByName('luminosidade').Value) then
  ...
  begin
    //ShowMessage('Você selecionou a mesma luminosidade que estava!');
    btnSalvar.Enabled := False;
  end;
end;
